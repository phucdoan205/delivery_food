...
import { ArrowLeft, Bell, ThumbsUp, ThumbsDown, Star } from 'lucide-react-native';
import { Colors } from '../../constants/colors';
import { CUSTOMER_REVIEWS, REVIEW_SUMMARY } from '../../constants/profileMockData';
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.subtitle}>Lắng nghe và tương tác để nâng cao chất lượng dịch vụ.</Text>
        ...
        <View style={[styles.metricCard, styles.positiveCard]}>
          <View>
            ...
            <Text style={styles.metricValue}>{REVIEW_SUMMARY.positiveCount}</Text>
        ...
        <View style={[styles.metricCard, styles.negativeCard]}>
          <View>
            ...
            <Text style={styles.metricValue}>{REVIEW_SUMMARY.negativeCount}</Text>
        ...
        {CUSTOMER_REVIEWS.map((item) => (
          <View key={item.id} style={[styles.reviewCard, item.needsAttention && styles.reviewCardAlert]}>
                ...
                <View style={styles.reviewerMeta}>
                  ...
                  <View style={styles.inlineStars}>
                    {Array.from({ length: item.rating }).map((_, index) => (
                      <Star key={index} size={12} color="#F5A623" fill="#F5A623" />
                    ))}
                  </View>
                </View>
              </View>
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 13, color: Colors.textSecondary, marginTop: 8, lineHeight: 20, marginBottom: 18 },
  summaryBlock: { marginBottom: 18 },
  summaryLabel: { fontSize: 13, color: Colors.textSecondary },
  ratingRow: { flexDirection: 'row', alignItems: 'center', marginTop: 6 },
  ratingValue: { fontSize: 42, fontWeight: '900', color: Colors.primary, marginRight: 10 },
  starsRow: { flexDirection: 'row' },
  summaryHint: { fontSize: 12, color: Colors.textSecondary, marginTop: 4 },
  metricCard: { borderRadius: 24, padding: 18, marginBottom: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' },
  ...
  metricValue: { fontSize: 34, fontWeight: '900', color: Colors.text, marginTop: 4 },
  ...
  reviewCard: { backgroundColor: Colors.white, borderRadius: 24, padding: 18, borderWidth: 1, borderColor: Colors.border, marginTop: 10 },
  ...
  replyBox: { backgroundColor: '#FFF1EC', borderRadius: 18, padding: 14, marginTop: 14 },
  replyLabel: { fontSize: 12, fontWeight: '700', color: Colors.primary, marginBottom: 6 },
  replyText: { fontSize: 12, lineHeight: 18, color: Colors.textSecondary },
  actionRow: { flexDirection: 'row', marginTop: 14 },
  primaryAction: { backgroundColor: Colors.primary, borderRadius: 18, paddingHorizontal: 16, paddingVertical: 10, marginRight: 10 },
  primaryActionText: { color: Colors.white, fontSize: 12, fontWeight: '700' },
  secondaryAction: { backgroundColor: '#FFE8E2', borderRadius: 18, paddingHorizontal: 16, paddingVertical: 10 },
